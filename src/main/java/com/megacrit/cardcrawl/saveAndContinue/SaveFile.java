...
/* loaded from: desktop-1.0.jar:com/megacrit/cardcrawl/saveAndContinue/SaveFile.class */
public class SaveFile {
    ...
    /* loaded from: desktop-1.0.jar:com/megacrit/cardcrawl/saveAndContinue/SaveFile$SaveType.class */
    ...
    public SaveFile(SaveType type) {
        ...
        Iterator<AbstractRelic> it = p.relics.iterator();
        while (it.hasNext()) {
        ...
        Iterator<AbstractBlight> it2 = p.blights.iterator();
        while (it2.hasNext()) {
            AbstractBlight b = it2.next();
            this.blights.add(b.blightID);
            this.blight_counters.add(Integer.valueOf(b.counter));
        }
        this.endless_increments = new ArrayList<>();
        Iterator<AbstractBlight> it3 = p.blights.iterator();
        while (it3.hasNext()) {
            this.endless_increments.add(Integer.valueOf(it3.next().increment));
        }
        this.potion_slots = AbstractDungeon.player.potionSlots;
        this.potions = new ArrayList<>();
        Iterator<AbstractPotion> it4 = AbstractDungeon.player.potions.iterator();
        while (it4.hasNext()) {
            AbstractPotion pot = it4.next();
        ...
        this.event_chances = type == SaveType.POST_COMBAT ? EventHelper.getChancesPreRoll() : EventHelper.getChances();
        this.save_date = Calendar.getInstance().getTimeInMillis();
        this.seed = Settings.seed.longValue();
        if (Settings.specialSeed != null) {
            this.special_seed = Settings.specialSeed.longValue();
        }
        ...
        switch (type) {
            case POST_COMBAT:
                ...
                Iterator<RewardItem> it5 = AbstractDungeon.getCurrRoom().rewards.iterator();
                while (it5.hasNext()) {
                    RewardItem i = it5.next();
                    switch (i.type) {
                        case SAPPHIRE_KEY:
                        case EMERALD_KEY:
                        case CARD:
        ...
        } else if (((BottledFlame) AbstractDungeon.player.getRelic(BottledFlame.ID)).card != null) {
            this.bottled_flame = ((BottledFlame) AbstractDungeon.player.getRelic(BottledFlame.ID)).card.cardID;
        } else {
            this.bottled_flame = null;
        }
        if (!AbstractDungeon.player.hasRelic(BottledLightning.ID)) {
            this.bottled_lightning = null;
        } else if (((BottledLightning) AbstractDungeon.player.getRelic(BottledLightning.ID)).card != null) {
            this.bottled_lightning = ((BottledLightning) AbstractDungeon.player.getRelic(BottledLightning.ID)).card.cardID;
        } else {
            this.bottled_lightning = null;
        }
        if (!AbstractDungeon.player.hasRelic(BottledTornado.ID)) {
            this.bottled_tornado = null;
        } else if (((BottledTornado) AbstractDungeon.player.getRelic(BottledTornado.ID)).card != null) {
            this.bottled_tornado = ((BottledTornado) AbstractDungeon.player.getRelic(BottledTornado.ID)).card.cardID;
        } else {
            this.bottled_tornado = null;
        }
